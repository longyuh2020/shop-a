      ...
      <el-form :model="form" label-width="100px" :rules="rules" ref="ruleForm">
        ...
        <el-form-item label="状态">
          <div v-for="(item,index) in attrsList" :key="index">
            <el-input v-model="item.value" class="cur"></el-input>
            <el-button type="success" v-if="index===0" @click="addAttr">添加属性</el-button>
            <el-button type="danger" v-else @click="deleAttr(index)">删除属性</el-button>
          </div>
        </el-form-item>
        <el-form-item label="状态">
  ...
  methods: {
    ...
    submitFn() {
      ...
      this.$refs.ruleForm.validate(async value => {
        console.log(value);
        if (value) {
          let attrs = this.attrsList.map(item=>item.value).join(',');
    ...
    async editFn(id) {
      ...
      this.attrsList = res.list[0].attrs.map(item=>({value:item}))
    }
  },
  created() {
    this.getTable();
